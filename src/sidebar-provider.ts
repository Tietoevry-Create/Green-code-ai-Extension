...
export class SidebarProvider implements vscode.WebviewViewProvider {
    ...
    webviewView.webview.onDidReceiveMessage(async (data) => {
      switch (data.type) {
        case "onRegister":
          ...
          const {
            azureOpenaiCompletionsDeploymentName,
            azureOpenaiBaseUrl,
            openaiApiEncryptionSalt,
            openaiApiPassword,
            openaiApiPasswordHash,
            openaiApiKey,
        ...
        case "onEdit": {
          ...
          if (openaiApiPassword) {
            let openaiApiEncryptionSalt: string | undefined =
              this._context?.globalState.get("openaiApiEncryptionSalt");
            let openaiApiKey = "";
            let newOpenaiApiPassword =
              openaiApiPassword == "nopassword" ? "" : openaiApiPassword;
            let newKey256Bits = CryptoJS.PBKDF2(
              newOpenaiApiPassword,
              openaiApiEncryptionSalt || "",
              { keySize: 256 / 32 }
            ).toString();
            let openaiApiPasswordHash = new Hashmap(
              newOpenaiApiPassword
            ).stringHash();

            if (unencryptedApiKey) {
              openaiApiKey = new AESEncryption(
                unencryptedApiKey,
                newKey256Bits
              ).encrypt();
            } else {
              let openaiApiEncryptedKey: string | undefined = "";
              await this._context?.secrets.get("openaiApiKey").then((key) => {
                openaiApiEncryptedKey = key;
              });
              let openaiApiPassword: string | undefined = "";
              await this._context?.secrets
                .get("openaiApiPassword")
                .then((password) => {
              ...
              let decryptedApiKey = new AESEncryption(
                openaiApiEncryptedKey || "",
                key256Bits.toString(CryptoJS.enc.Hex)
              ).decrypt();
              openaiApiKey = new AESEncryption(
                decryptedApiKey,
                newKey256Bits
            ...
            if (
              openaiApiPasswordCreatedDate &&
              openaiApiPasswordCreatedDate != 0
            ) {
              this._context?.globalState.update(
                "openaiApiPasswordCreatedDate",
                openaiApiPasswordCreatedDate
            ...
            const openaiApiKey = new AESEncryption(
              unencryptedApiKey,
              key256Bits
            ).encrypt();
            this._context?.secrets.store("openaiApiKey", openaiApiKey);
        ...
        case "onCheckApiKey": {
          ...
          let openaiApiEncryptionSalt: string | undefined =
            this._context?.globalState.get("openaiApiEncryptionSalt");
          let openaiApiPasswordCreatedDate: number | undefined =
            this._context?.globalState.get("openaiApiPasswordCreatedDate");
          await this._context?.secrets.get("openaiApiKey").then((key) => {
            openaiApiKey = key;
          });
          await this._context?.secrets
          ...
          await this.context?.secrets
            .get("openaiApiPasswordHash")
            .then((hash) => {
              openaiApiPasswordHash = hash;
            });
          ...
          if (openaiApiPasswordCreatedDate) {
            ...
            daysSinceCreated =
              (currentDate - openaiApiPasswordCreatedDate) / 86400000;
          } else {
            daysSinceCreated = 0;
          }
          ...
          if (
            ...
            this._view?.webview.postMessage({
              type: "onApiKeyExists",
              value: [openaiApiPassword ? true : false, daysSinceCreated],
            });
            console.log("API key exists");
          } else {
            this._view?.webview.postMessage({
              type: "onApiKeyNotFound",
              value: "",
            });
        ...
        case "onCheckPassword": {
          ...
          if (openaiApiPasswordHash == data.value) {
            this._view?.webview.postMessage({
              type: "onCorrectPassword",
              value: "",
            });
          } else {
            this._view?.webview.postMessage({
              type: "onIncorrectPassword",
              value: "",
            });
          }
          break;
        ...
        case "onFetchText": {
          ...
          let azureOpenaiCompletionsDeploymentName: string | undefined =
            this.context?.globalState.get(
              "azureOpenaiCompletionsDeploymentName"
            );
          let azureOpenaiBaseUrl: string | undefined =
            this.context?.globalState.get("azureOpenaiBaseUrl");
          let openaiApiEncryptionSalt: string | undefined =
            this._context?.globalState.get("openaiApiEncryptionSalt");
          let openaiApiEncryptedKey: string | undefined = "";
          ...
          let apiKey = new AESEncryption(
            openaiApiEncryptedKey || "",
            key256Bits.toString(CryptoJS.enc.Hex)
          ).decrypt();
          let aiIntegration = new AIIntegration(
            azureOpenaiCompletionsDeploymentName || "",
            azureOpenaiBaseUrl || "",
            apiKey || "",
            this._context
          ...
          try {
            [formattedText, lineNumbers] = new TextFormatter().formatText(
              text,
              this._highlightLines ? this._highlightColorList : []
            );
          } catch (error) {
            if (this._errorRetry < 2) {
              this._view?.webview.postMessage({
                type: "onRegenResponse",
                value: "",
              });
              this._errorRetry++;
            } else {
    ...
    lineNumbers.forEach((lineNumberSet) => {
      ...
      if (decorationRanges.length > 0) {
        editor!.setDecorations(
          this._highlightDecorationTypeList[decorationCounter],
          decorationRanges
        );
        decorationCounter =
          (decorationCounter + 1) % this._highlightDecorationTypeList.length;
      }
    });
  }
  ...
  private onDidChangeTextEditorSelection(
    _event: vscode.TextEditorSelectionChangeEvent
  ) {
    this.clearAllHighlights();
  }
}
